...
func (e EnergyKJ) GetPoints(st ScoreType) int {
	if st == Beverage {
		return getPointFromRange(float64(e), energyLevelsBeverage)
	}
	return getPointFromRange(float64(e), energyLevels)
}

func (s SugarGram) GetPoints(st ScoreType) int {
	if st == Beverage {
		return getPointFromRange(float64(s), sugarsLevelsBeverage)
	}
	return getPointFromRange(float64(s), sugarsLevels)
}

func (sfa SaturatedFattyAcids) GetPoints(st ScoreType) int {
	return getPointFromRange(float64(sfa), saturatedFattyAcidsLevels)
}

func (s SodiumMilligram) GetPoints(st ScoreType) int {
	return getPointFromRange(float64(s), sodiumLevels)
}
...
func (f FibreGram) GetPoints(st ScoreType) int {
	return getPointFromRange(float64(f), fibreLevels)
}

func (p ProteinGram) GetPoints(st ScoreType) int {
	return getPointFromRange(float64(p), proteinLevels)
}
...
func (ns NutritionalScore) GetNutriscore() string {
	if ns.ScoreType == Food {
		return scoreToLetter[getPointFromRange(float64(ns.Value), []float64{18, 10, 2, -1})]
	}
	if ns.ScoreType == Water {
		return scoreToLetter[0]
	}
	return scoreToLetter[getPointFromRange(float64(ns.Value), []float64{9, 5, 2, -1})]
}

func getPointFromRange(v float64, steps []float64) int {
	lenSteps := len(steps)
	for i, l := range steps {
		if v > l {
			return lenSteps - i
